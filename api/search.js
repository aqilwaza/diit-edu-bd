// API endpoint for SQL Injection and XSS vulnerability testing
...
<body>
  ...
  <p>No search query provided. Use ?q=your_query</p>
    ...
    // Check for valid XSS payloads (case-insensitive partial match)
    const isValidXSS = validXSSPayloads.some(payload => 
        query.includes(payload) || 
        query.toLowerCase().includes(payload.toLowerCase())
    )

    if (isValidXSS) {
        // VULNERABLE: XSS - Return query without sanitization
        return res.status(200).send(`
<!DOCTYPE html>
<html>
<head><title>Search Results - XSS</title></head>
<body>
